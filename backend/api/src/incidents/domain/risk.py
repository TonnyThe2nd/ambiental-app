...
class RiskAssessmentService:
    """Pure, deterministic domain policy for environmental triage."""

    _base_risk = {"alagamento": 65, "poluicao": 60, "queimada": 80,
                  "desmatamento": 70, "esgoto": 65, "lixo": 35, "ruido": 25}

    def assess(self, category: str, *, sensitive_area_criticality: int = 0,
               rainfall_mm: float | None = None, air_quality_index: int | None = None) -> RiskAssessment:
        normalized = category.strip().lower()
        score = float(self._base_risk.get(normalized, 40))
